...
if (! nv_function_exists('nv_block_news_groups_nivo_slider')) {
    function nv_block_config_news_groups_nivo_slider($module, $data_block, $lang_block)
    {
		...
		foreach( $list as $l )
		{
			...
			$block .= nv_get_blocklist( $l['title'] );
		}
	...
	function nv_get_blocklist( $module )
	{
    ...
    function nv_block_config_news_groups_nivo_slider_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = array();
        $return['error'] = array();
    ...
    function nv_block_news_groups_nivo_slider($block_config)
    {
        ...
        if (! empty($list)) {
            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/blocks/block_groups_nivo_slider.tpl')) {
                $block_theme = $global_config['module_theme'];
            } else {
                $block_theme = 'default';
            }
            $xtpl = new XTemplate('block_groups_nivo_slider.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/blocks');
            ...
            foreach ($list as $l) {
				...
				if( file_exists( NV_UPLOADS_DIR . '/' . $site_mods[$module]['module_upload'] . '/' . $l['homeimgfile'] ) ){
					$l['homeimgfile'] = nv_resize_crop_images( NV_UPLOADS_REAL_DIR . '/' . $site_mods[$module]['module_upload'] . '/' . $l['homeimgfile'], 600, 300, $module, $l['id'] );
				}
...
if (defined('NV_SYSTEM')) {
    ...
    if (isset($site_mods[$module])) {
        ...
        $content = nv_block_news_groups_nivo_slider($block_config);
    }
}