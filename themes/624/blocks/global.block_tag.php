...
if (! nv_function_exists('nv_news_tag')) {
    ...
    function nv_news_tag($block_config)
    {
        ...
        $db->sqlreset()
            ...
            ->order('tid DESC') //DESC ASC
		//	`status`=1 AND `inhome`='1' ORDER BY RAND() LIMIT 0 ,  rand() limit 0,3
            ->limit($tags_number);
...
if (defined('NV_SYSTEM')) {
    ...
    if (isset($site_mods[$module])) {
        if ($module == $module_name) {
            ...
            $sql = 'SELECT catid, parentid, title, alias, viewcat, subcatid, numlinks, description, status, keywords, groups_view FROM ' . NV_PREFIXLANG . '_' . $site_mods[$module]['module_data'] . '_cat ORDER BY RAND() LIMIT 0,6 '; //ORDER BY sort ASC
