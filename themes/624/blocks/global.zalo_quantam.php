...
if (! nv_function_exists('nv_menu_theme_quantam')) {
    function nv_menu_theme_quantam_config($module, $data_block, $lang_block)
    {
        $html = '<tr>';
		...
		<a href="http://developers.zalo.me/docs/" target="_blank"><span itemprop="url">tại đây</span></a>
		</td>';
        $html .= '</tr>';
    ...
    function nv_menu_theme_quantam_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = array();
        $return['error'] = array();
    ...
    function nv_menu_theme_quantam($block_config)
	 {
        global $global_config, $site_mods, $lang_global, $module_file, $client_info, $op;

        if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/blocks/global.zalo_quantam.tpl')) {
            $block_theme = $global_config['module_theme'];
        } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/blocks/global.zalo_quantam.tpl')) {
        ...
        $xtpl = new XTemplate('global.zalo_quantam.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/blocks');
...
if (defined('NV_SYSTEM')) {
    $content = nv_menu_theme_quantam($block_config);
}
